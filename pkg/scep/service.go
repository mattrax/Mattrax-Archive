...
import (
	...
	"log" //TODO: Remove Logging -> Everything Should be Returned

	"github.com/mattrax/scep/scep"
...
func (svc *service) PKIOperation(ctx context.Context, data []byte) ([]byte, error) {
	msg, err := scep.ParsePKIMessage(data) //scep.WithLogger(svc.debugLogger) //TODO: Come Back Here
	if err != nil {
		return nil, err
	}
	ca := svc.ca[0]
	...
	if msg.MessageType == scep.PKCSReq {
		...
		if svc.csrVerifier != nil {
			...
			CSRIsValid = result
			if !CSRIsValid {
				log.Println("err", "CSR is not valid")
			}
		} else {
			CSRIsValid = svc.challengePasswordMatch(msg.CSRReqMessage.ChallengePassword)
			if !CSRIsValid {
				log.Println("err", "scep challenge password does not match")
			}
		}
...
func (svc *service) challengePasswordMatch(pw string) bool {
	...
	if svc.supportDynamciChallenge {
		/*valid, err := svc.dynamicChallengeStore.HasChallenge(pw) //TODO: Come Back Here
		if err != nil {
			log.Println(err)
			return false
		}*/
		log.Println("Challenge Password: ", pw)
		return true //valid
	}
